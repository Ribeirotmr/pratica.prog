...
def escolheDestino(n):
    ...
    # usuário o destino enquanto ele digitar um valor inválido (não-inteiro)
    # ou um destino inválido
    try:
...
def programa(i=1):
    ...
    if resp == "S" or resp == "s":
        ...
        print(
            f"Finalizando o programa após {i} execuç{'ões' if i > 1 else 'ão'}"
        )  # Note a utilização de expressão ternária
